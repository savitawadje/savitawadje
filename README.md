<div align="center">

# 👋 Hey, I'm Savita Wadje

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=26&pause=1000&color=DD0031&center=true&vCenter=true&width=650&lines=Frontend+Developer+%7C+Angular+%26+React;Building+UIs+that+users+love;5%2B+Years+of+Production+Experience;Werkstudent+%40+TU+Chemnitz+%F0%9F%87%A9%F0%9F%87%AA" alt="Typing SVG" />

</div>

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/savita-wadje-85b50b368)
[![Dev.to](https://img.shields.io/badge/dev.to-0A0A0A?style=for-the-badge&logo=devdotto&logoColor=white)](https://dev.to/savitawadje)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:099savitawadje4@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/savitawadje)


</div>

---

## 🎯 About Me

```typescript
class SavitaWadje {
  role        = "Frontend Developer (Werkstudent)";
  location    = "Chemnitz, Germany 🇩🇪";
  education   = "M.Sc. Web Engineering @ TU Chemnitz (2025–)";
  experience  = "5+ years | Cybage · LTIMindtree · Codesplit";
  passion     = "Turning Figma wireframes into pixel-perfect UIs";

  stack() {
    return {
      primary:   ["Angular", "TypeScript", "JavaScript", "Tailwind CSS"],
      learning:  ["React — building projects to expand beyond Angular"],
      backend:   ["Node.js", "Express.js", "REST APIs"],
      databases: ["MongoDB", "PostgreSQL", "MySQL"],
      devops:    ["AWS", "Docker", "CI/CD", "Jenkins", "Kubernetes"],
      tools:     ["Figma", "Git", "GitHub", "Postman", "Swagger"],
    };
  }

  currentFocus() {
    return "Combining 5 years of production engineering with M.Sc. depth";
  }

  lookingFor() {
    return "Werkstudent Frontend roles in Germany & Europe 🌍";
  }
}
```

<div align="center">

### 🚀 Right Now

🎓 Pursuing **M.Sc. Web Engineering** at **TU Chemnitz**  
⚡ Actively learning **React** — building personal projects  
💼 Available **15–20 hrs/week** during semester · Full-time during breaks  
🌍 Open to **Werkstudent roles** across **Germany & Europe**

</div>

---

## 💼 What I Bring to Your Team

<table>
<tr>
<td width="50%">

### 🎯 Production-Proven Frontend
- ✅ **2+ apps** built from scratch at Cybage (Figma → production)
- ✅ **10+ reusable components** with optimised data binding
- ✅ Cross-browser compatible UIs across **4+ major browsers**
- ✅ Enterprise Angular apps at **LTIMindtree & Cybage**

</td>
<td width="50%">

### 🔬 Engineering Quality
- ✅ **30+ components** unit tested & debugged at LTIMindtree
- ✅ **15+ UI components** with standardised CSS at Codesplit
- ✅ REST API integration across complex backend systems
- ✅ Microfrontend architecture experience

</td>
</tr>
</table>

<table>
<tr>
<td width="50%">

### ☁️ Cloud & DevOps Awareness
- ✅ Cloud deployment on **AWS & Azure**
- ✅ Docker containerisation & CI/CD pipelines
- ✅ Jenkins, Kubernetes — not just a UI developer
- ✅ End-to-end feature delivery in Agile sprints

</td>
<td width="50%">

### 🤝 Collaboration & Communication
- ✅ Worked cross-functionally with design & backend teams
- ✅ Translated stakeholder requirements into working UIs
- ✅ English (C1 Proficient) · German (B1 in progress)
- ✅ Agile · Scrum · Sprint Planning · Code Reviews

</td>
</tr>
</table>

---

## 🛠️ Tech Arsenal

<div align="center">

### ⚡ Frontend
![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React_(learning)-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38BDF8?style=for-the-badge&logo=tailwindcss&logoColor=white)
![Sass](https://img.shields.io/badge/Sass-CC6699?style=for-the-badge&logo=sass&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### 🔧 Backend & APIs
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express.js](https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST_APIs-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=for-the-badge&logo=postgresql&logoColor=white)

### ☁️ Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=for-the-badge&logo=jenkins&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

### 🎨 Design & Tools
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white)
![Angular Material](https://img.shields.io/badge/Angular_Material-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)

</div>

---

## 🚀 Featured Projects

<table>
<tr>
<td width="50%">

### ⚙️ Enterprise Angular Applications
<img src="https://img.shields.io/badge/Status-Production-success?style=for-the-badge" alt="Production"/>

**Scalable web apps · Cybage Software**

🎯 Built 2+ applications from scratch in Agile teams  
🎨 Figma wireframes → production-ready Angular architecture  
🔌 Seamless REST API integration with .NET & Java backends  
🌐 Cross-browser compatible across 4+ major browsers

**Tech:** Angular · TypeScript · HTML5 · CSS3 · REST APIs · AWS

</td>
<td width="50%">

### 🧩 Reusable Component Library
<img src="https://img.shields.io/badge/Status-Shipped-blue?style=for-the-badge" alt="Shipped"/>

**Modular UI architecture · LTIMindtree**

🎯 Developed 10+ reusable Angular components  
🔍 Efficient data binding & state management  
🧪 30+ components unit tested & debugged  
🏗️ Contributed to microfrontend architecture

**Tech:** Angular · Angular Material · TypeScript · RxJS · NgRx

</td>
</tr>
</table>

<table>
<tr>
<td width="50%">

### 🌐 React Learning Projects
<img src="https://img.shields.io/badge/Status-In_Progress-orange?style=for-the-badge" alt="In Progress"/>

**Expanding beyond Angular — actively building**

🎯 Hands-on React projects while studying at TU Chemnitz  
⚛️ Applying TypeScript & component patterns from Angular experience  
🎨 Tailwind CSS for rapid UI development  
📚 Exploring React ecosystem: hooks, context, routing

**Tech:** React · TypeScript · Tailwind CSS · JavaScript

</td>
<td width="50%">

### 📐 CSS Architecture & Layouts
<img src="https://img.shields.io/badge/Status-Delivered-success?style=for-the-badge" alt="Delivered"/>

**Standardised UI systems · Codesplit Technologies**

🎯 Developed 15+ UI components from specs  
🌐 Cross-browser compatibility across 3+ browsers  
🎨 Standardised CSS-based layout systems  
🔌 API-integrated data binding across frontend layers

**Tech:** Angular · Sass · HTML5 · CSS3 · Bootstrap · Node.js

</td>
</tr>
</table>

---

## 📊 GitHub Analytics

<div align="center">

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=savitawadje&show_icons=true&theme=radical&hide_border=true" />
<img width="49%" src="https://github-readme-streak-stats.herokuapp.com/?user=savitawadje&theme=radical&hide_border=true" />

<img width="49%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=savitawadje&layout=compact&theme=radical&hide_border=true" />
<img width="49%" src="https://github-readme-activity-graph.vercel.app/graph?username=savitawadje&theme=redical&hide_border=true&area=true" />

</div>



## 💼 Professional Experience

<table>
<tr>
<td width="30%">

![Cybage](https://img.shields.io/badge/Cybage_Software-0057A8?style=for-the-badge&logoColor=white)

**Apr 2024 – Sep 2025**

*Senior Software Engineer*

Pune, India

</td>
<td width="70%">

**Key Contributions:**
- 🎯 Built 2+ web applications from scratch — Figma to production-ready Angular architecture
- 🌐 Delivered responsive, cross-browser UIs across 4+ major browsers in Agile sprints
- 🧩 Developed 10+ modular, reusable components with efficient data binding & REST API integration
- ☁️ Integrated frontends with .NET & Java backends deployed on AWS and Azure

</td>
</tr>
</table>

<table>
<tr>
<td width="30%">

![LTIMindtree](https://img.shields.io/badge/LTIMindtree-6DB33F?style=for-the-badge&logoColor=white)

**Apr 2021 – May 2023**

*Software Engineer*

Bangalore, India

</td>
<td width="70%">

**Key Contributions:**
- 🎯 Developed user-facing Angular applications with Angular Material for enterprise clients
- 🏗️ Contributed to microfrontend architecture within 2 existing enterprise applications
- 🧪 Unit tested and debugged 30+ components, maintaining stability across production releases
- 🤝 Collaborated cross-functionally with design and backend teams to resolve critical system issues

</td>
</tr>
</table>

<table>
<tr>
<td width="30%">

![Codesplit](https://img.shields.io/badge/Codesplit_Technologies-FF6B35?style=for-the-badge&logoColor=white)

**Dec 2019 – Apr 2021**

*Software Engineer*

Pune, India

</td>
<td width="70%">

**Key Contributions:**
- 🎯 Developed 15+ reusable UI components with API-integrated data binding
- 🌐 Created standardised CSS layouts ensuring cross-browser compatibility across 3+ browsers
- 🏗️ Built responsive interfaces using Angular, TypeScript, and modern CSS practices
- 📐 Established component and CSS standards used across team projects

</td>
</tr>
</table>

---

## 🎓 Education

<div align="center">

### 🎓 Technische Universität Chemnitz
**Master of Science in Web Engineering** | 🇩🇪 Germany

*Focus:* Modern Web Architectures · Scalable Frontend Systems · Web Engineering

---

### 🎓 JSPM, Savitribai Phule Pune University
**Bachelor of Engineering — Computer Science** | 🇮🇳 India

</div>

---

## 🌍 Languages

<div align="center">

| Language | Proficiency | Level |
|:---:|:---:|:---:|
| 🇮🇳 **Marathi** | Native | C2 |
| 🇮🇳 **Hindi** | Native | C2 |
| 🇬🇧 **English** | Proficient | C1 |
| 🇩🇪 **German** | Learning | B1 (in progress) |

</div>

---

## 💡 Why Work With Me?

```
🧱 I don't just write components — I architect them to be reusable and maintainable
⚡ I move fast in Agile teams without cutting corners on quality
🌉 I bridge the gap between design and engineering — Figma to production
🔬 I'm actively expanding my stack (React) while studying Web Engineering
🤝 I communicate clearly — in code, in standups, in English and improving German
📍 Based in Chemnitz, Germany — ready to contribute from day one
```

---

## 📞 Let's Build Something Together

<div align="center">

### 💼 Open to Opportunities

🖥️ **Werkstudent Frontend** | ⚛️ **Angular / React Projects** | 🌍 **Germany & Europe**

---

<a href="https://linkedin.com/in/savita-wadje-85b50b368" target="_blank">
  <img src="https://img.shields.io/badge/Connect_on_LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
</a>
<a href="mailto:099savitawadje4@gmail.com" target="_blank">
  <img src="https://img.shields.io/badge/Send_Me_an_Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
</a>

---

### ⏰ Available 15–20 hrs/week during semester · Full-time during breaks

*Based in Chemnitz, Germany (CET timezone)*

---

<img src="https://capsule-render.vercel.app/api?type=waving&color=DD0031&height=100&section=footer" width="100%"/>

### 💭 *"5 years of production engineering — now going deeper."*

![Angular](https://img.shields.io/badge/Powered_by-Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![TypeScript](https://img.shields.io/badge/Built_with-TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![Made with Love](https://img.shields.io/badge/Made_with-❤️-red?style=for-the-badge)

</div>
